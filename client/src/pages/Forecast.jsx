...
import { CloudSun, Thermometer, Droplets, Wind, CloudRain, MapPin, ChevronDown, ChevronUp, ArrowLeft, HeartPulse } from 'lucide-react';
...
const Forecast = () => {
    ...
    const handleFieldSelect = async (field) => {
        ...
        try {
            ...
            // Fallback for demo mode if explicit endpoint not yet implemented in routes
            const demoWeather = {
                temperature: 28, humidity: 75, condition: "Clear Sky",
                wind_speed: 3.5, rain_chance: 30, location: field.location || "Your Farm"
            };
            setWeather(demoWeather);
            setRisks([
                { name: "Powdery Mildew", risk_level: "medium", risk_score: 55, reason: "Moderate humidity (75%) and warm temperatures (28°C) favor spore growth.", prevention: ["Ensure air movement", "Avoid overhead water", "Use sulfur spray"], crops: ["Tomato", "Cucumber"] },
                { name: "Late Blight", risk_level: "low", risk_score: 20, reason: "Conditions are currently too dry for blight development.", prevention: ["Proper drainage", "Clean seeds"], crops: ["Potato", "Tomato"] }
            ]);
        } finally {
            setLoading(false);
        }
    ...
    return (
        <div className="min-h-screen bg-[#f8faf8] p-6 lg:p-10">
            <div className="max-w-6xl mx-auto flex justify-between items-center mb-8">
                ...
                {fields.length > 0 && (
                    <div className="flex items-center gap-3">
                        <MapPin className="text-[#6fb342]" />
                        <select
                            className="bg-white border-2 border-green-100 rounded-2xl px-6 py-3 font-bold text-[#2d5a27] outline-none shadow-sm focus:border-green-400 appearance-none min-w-[200px]"
            ...
            {loading ? (
                <div className="flex flex-col items-center justify-center py-40 animate-pulse">
                    ...
                    <h2 className="text-2xl font-bold text-green-900/40 tracking-widest">ANALYZING ATMOSPHERE...</h2>
                </div>
            ) : weather ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            {risks.map((risk, idx) => (
                                <div key={idx} className="bg-[#fcfdfc] border border-green-100 p-8 rounded-[40px] hover:shadow-lg transition duration-500">
                                    ...
                                    >
                                        Prevention Protocol {expandedTips[idx] ? <ChevronUp size={18} /> : <ChevronDown size={18} className="translate-y-px" />}
                                    </button>
